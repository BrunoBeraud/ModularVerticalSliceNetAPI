...
public class FunctionalDomainsTests
{
    ...
    private readonly IObjectProvider<IType> _coreLayer =
        Types().That().ResideInNamespace(@".*\.Core.*", useRegularExpressions: true);

    private readonly IObjectProvider<Class> _featuresLayer =
        Classes().That().ResideInNamespace(@"\..*Features\.", useRegularExpressions: true);

    private readonly IObjectProvider<Class> _useCasesLayer =
        Classes().That().HaveNameEndingWith("UseCase");

    private readonly IObjectProvider<Class> _endpointsLayer =
        Classes().That().HaveNameEndingWith("Endpoint");
    private readonly IObjectProvider<IType> _dtosLayer =
        Types().That().HaveNameEndingWith("Request")
            .Or().HaveNameEndingWith("Response")
            .Or().HaveNameEndingWith("Error")
    ...
    public void CoreOnlyDependsOnDomainRelatedTypes()
    {
        Types().That().Are(_coreLayer)
           .Should().OnlyDependOn(Types().That().Are(_coreLayer)
               .Or().ResideInAssembly(AssembliesRepository.SharedKernel))
            .Because("Core, as a guarantor of business logic, must not depend on infrastructure details")
            .Check(Architecture);
    }
    ...
    {
        Types().That().Are(_useCasesLayer)
           .Should().OnlyDependOn(Types().That().Are(_coreLayer)
               .Or().ResideInAssembly(AssembliesRepository.SharedKernel))
            .Because("As the bearer of business application logic, UseCase must not depend on infrastructure details")
            .Check(Architecture);
    }
    ...
    {
        var classesByFeature = Classes().That().Are(_featuresLayer).GetObjects(Architecture)
            .GroupBy(classesInFeaturesLayer => classesInFeaturesLayer.Namespace.FullName);

        Assert.All(classesByFeature, featureClasses =>
            Assert.Single(collection: featureClasses, predicate: featureClass => featureClass.FullName.EndsWith("UseCase")));
    }
    ...
    public void EachFeatureHaveAtLeastOneEndpoint()
    {
        ...
        Assert.All(classesByFeature, featureClasses =>
            Assert.True(
                featureClasses.Where(featureClass => featureClass.FullName.EndsWith("Endpoint")).Any(),
    ...
    public void EachEndpointReferenceOnlyOneUseCase()
    {
        ...
        var endpointsByNumberOfUseCaseReferenced = endpoints
            .Select(endpoint => (
                endpoint,
                UseCases: endpoint.Dependencies
                    .Select(x => x.Target.FullName)
                    .Where(x => x.EndsWith("UseCase"))
                    .Distinct()));

        Assert.All(endpointsByNumberOfUseCaseReferenced, endpointClassWithNumberUseCaseReferenced =>
            Assert.True(endpointClassWithNumberUseCaseReferenced.UseCases.Count() == 1,
    ...
    public void DtosAreRecords()
    {
        ...
        Assert.All(dtosTypes, dtoType => Assert.True(dtoType.IsRecord, $"{dtoType.FullName} Dto is not a record"));
    }
    ...
    public void AllTypesAreInternals()
    {
        Types().That().ResideInAssembly(
            ...
            .And().AreNot(typeof(FunctionalDomainNameARegistration), typeof(FunctionalDomainNameBRegistration))
            .Should().BeInternal()
            .Because("functional domain must be considered as an isolated module that could be its own independent component")
            .Check(Architecture);
    }
}