...
public class SharedKernelTests
{
    private static readonly Architecture Architecture = new ArchLoader()
        .LoadAssemblies(
            [.. AssembliesRepository.FunctionalDomains, AssembliesRepository.SharedKernel]
        )
        .Build();

    private readonly IObjectProvider<IType> _sharedKernel = Types()
        .That()
        .ResideInAssembly(AssembliesRepository.SharedKernel);

    private readonly IObjectProvider<IType> _functionalDomainsLayer = Types()
        .That()
        .ResideInAssembly(
    ...
    public void SharedKernelTypesAreSharedByAtLeastTwoFunctionalDomain()
    {
        ...
        var functionalDomainsTypes = Types()
            .That()
            .Are(_functionalDomainsLayer)
